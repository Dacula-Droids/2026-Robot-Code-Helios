...
public class IntakeSubsystem extends SubsystemBase {
  ...
  public void motorConfigs() {
    TalonFXConfiguration intakeMotorConfig = new TalonFXConfiguration()
        ...
        .withCurrentLimits(
            new CurrentLimitsConfigs().withStatorCurrentLimit(Amps.of(120)).withStatorCurrentLimitEnable(true));
    ...
    // There are two encoder tick to mechanism rotation conversion methods in
    // TalonFX
    // RotorToSensorRatio is used when external encoder is used, or if encoder is
    // geared relative to the motor
    // SensorToMechanismRatio directly connects to the built-in encoder on the
    // talon, SparkMax equivalent to PositionConversionFactor and
    // VelocityConversionFactor

    FeedbackConfigs feedback = new FeedbackConfigs();
    feedback.SensorToMechanismRatio = 5.0; // This value for 1 : 5 gear ratio, meaning that 5 spins in motor result in
  ...
  public void intake() {
    // intakeMotor.set(0.5);
    intakeMotor.setControl(velocityRequest.withVelocity(15)); // RPS, Motor Velocity, not Wheel Velocity, Gear
                                                              // Reductions and Ratios must be converted
  }
